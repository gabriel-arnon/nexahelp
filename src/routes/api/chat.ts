...
type ChatGenerator = typeof generateAssistantAnswer;
type SanitizedChatMessage = { role: "user" | "assistant"; content: string };
...
export function selectUsefulModelHistory(
  ...
  const lastRelevantUserQuestion = [...usefulMessages]
    .reverse()
    .find((message) => message.role === "user");

  return lastRelevantUserQuestion ? [lastRelevantUserQuestion] : [];
}

export async function handleChatPost(
  request: Request,
  generateAnswer: ChatGenerator = generateAssistantAnswer,
): Promise<Response> {
  ...
  const localIntent = resolveLocalChatIntent(parsed.data.pergunta);
  if (localIntent) {
    return Response.json(localIntent);
  }

  const history = selectUsefulModelHistory(parsed.data.historico, parsed.data.pergunta);
  ...
  try {
    ...
    const resposta = await generateAnswer({
      apiKey,
    ...
    if (error instanceof ProviderIncompleteError) {
      return jsonError(
        502,
        "PROVIDER_INCOMPLETE",
        "A resposta ficou incompleta no provedor. Reformule a pergunta ou tente novamente em instantes.",
      );
    }
